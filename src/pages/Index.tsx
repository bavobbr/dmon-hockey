    ...
    <div className="min-h-screen bg-background overflow-x-hidden max-w-full">
          ...
          {user ? (
            <div className="space-y-4">
              <div className="text-center">
                ...
                {isAdmin && (
                  <span className="text-sm bg-accent text-accent-foreground px-3 py-1 rounded-full mr-2">Admin</span>
                )}
                {isModerator && !isAdmin && (
                  <span className="text-sm bg-secondary text-secondary-foreground px-3 py-1 rounded-full mr-2">
                    Moderator
                  </span>
                )}
              </div>
              <div className="flex gap-4 justify-center flex-wrap">
                ...
                <Button
                  ...
                  className="border-white/50 text-white bg-white/10 hover:bg-white/20 hover:text-white hover:border-white/70 px-8 py-4 backdrop-blur-sm"
                >
                  Afmelden
                </Button>
              </div>
      ...
      <section className="py-20 px-4 bg-gradient-subtle overflow-x-hidden">
        <div className="container mx-auto max-w-full">
          <h2 className="font-display text-4xl lg:text-5xl font-bold text-center mb-16 text-foreground">
            Wat We Bieden
          </h2>
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <Card className="border-accent/20 hover:border-accent/40 group fade-in-up">
              ...
              <CardContent>
                <p className="text-muted-foreground leading-relaxed">
                  Sluit je aan bij een ondersteunende gemeenschap van hockeyliefhebbers die je passie voor de sport
                  delen.
                </p>
              </CardContent>
            </Card>
                    ...
                    <Dialog>
                      <DialogTrigger asChild>
                        <Button
                          variant="outline"
                          size="sm"
                          className="group-hover:border-primary/50 group-hover:text-primary"
                        >
                          Lees Meer
                        </Button>
                      </DialogTrigger>
      ...
      <section className="py-20 px-4 bg-gradient-subtle">
        <div className="container mx-auto">
          <div className="flex items-center justify-between mb-16">
            <h2 className="font-display text-4xl lg:text-5xl font-bold text-center text-foreground">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {teams.map((team) => (
                <Card key={team.id} className="hover:shadow-lg transition-shadow">
                  ...
                  {team.description && (
                    <CardContent>
                      <p className="text-muted-foreground line-clamp-3">{team.description}</p>
                    </CardContent>
                  )}
                </Card>
              ))}
          ...
          {instagramLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {instagramPosts.map((post) => (
                <Card
                  key={post.id}
                  className="group cursor-pointer hover:shadow-lg transition-all duration-300 relative overflow-hidden"
                >
            ...
            <div className="text-center py-12">
              <p className="text-muted-foreground">Volg ons op Instagram voor de nieuwste updates!</p>
            </div>
          )}
        </div>
      </section>
          ...
          {sponsorsLoading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
              {sponsors.map((sponsor) => (
                <div
                  key={sponsor.id}
                  className="flex items-center justify-center p-4 bg-card rounded-lg border hover:shadow-lg transition-shadow"
                >
                  {sponsor.logo_path ? (
                    <img
                    ...
                    <div className="text-center text-sm font-medium text-muted-foreground">{sponsor.name}</div>
                  )}
                </div>
              ))}
            </div>
      ...
      <section className="bg-muted py-[14px]">
        <div className="container mx-auto px-4 text-center">
          <div className="flex justify-center gap-2 sm:gap-4 flex-wrap">
            <div className="w-12 h-12 sm:w-20 sm:h-20 bg-primary rounded-lg shadow-lg flex items-center justify-center"></div>
            <div className="w-12 h-12 sm:w-20 sm:h-20 bg-secondary rounded-lg shadow-lg flex items-center justify-center"></div>
            <div className="w-12 h-12 sm:w-20 sm:h-20 bg-accent rounded-lg shadow-lg flex items-center justify-center"></div>
            <div className="w-12 h-12 sm:w-20 sm:h-20 bg-foreground rounded-lg shadow-lg flex items-center justify-center"></div>
          </div>
        </div>
      </section>
    </div>

...
serve(async (req) => {
  ...
  try {
    ...
    if (limit !== undefined && (Number.isNaN(limit) || limit < 0)) {
      return new Response(
        JSON.stringify({ success: false, error: "Invalid limit parameter" }),
        {
          status: 400,
    ...
    let teams: TwizzitTeam[] = Array.isArray(teamsData)
      ? teamsData
      : teamsData.teams || [];
    ...
    const resolveContactName = async (
      ...
      log("contacts.request", { rid, url: contactsUrl, contactId });
    ...
    for (const team of teams) {
      ...
      log("group_contacts.request", { rid, url: groupContactsUrl, teamId: team.id });
      ...
      const getContactNamesByFunction = async (
        functionId: number
      ): Promise<string | null> => {
        const contacts = groupContacts.filter(
          (contact) => contact.contactFunctionId === functionId
        );
        if (contacts.length === 0) return null;

        const names = await Promise.all(
          contacts.map((contact) => resolveContactName(contact.contactId))
        );
